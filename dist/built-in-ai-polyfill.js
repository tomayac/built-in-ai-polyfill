  ...
  return t ? typeof t == "function" ? t() : Promise.resolve(t) : new Promise((n, s) => {
    (typeof queueMicrotask == "function" ? queueMicrotask : setTimeout)(
      s.bind(
        null,
        new Error(
          "Unknown variable dynamic import: " + a + (a.split("/").length !== e ? ". Note that variables only represent file names one level deep." : "")
        )
      )
    );
  });
...
class _ {
  constructor() {
    this.__polyfill = {
      async setBackend(a) {
        const [e, t] = a.split("/");
        w = (await R(/* @__PURE__ */ Object.assign({ "./ai-providers/cloud/chatgpt.js": () => import("./chatgpt-BbcDl7zQ.js"), "./ai-providers/cloud/gemini.js": () => import("./gemini-B15X68vq.js"), "./ai-providers/local/mediapipe-llm.js": () => import("./mediapipe-llm-BPKlf3M6.js") }), `./ai-providers/${e}/${t}.js`, 4)).default;
      }
    };
  }
  async create(a = {}) {
...
class v extends EventTarget {
  #a;
  constructor(a = {}) {
    if (super(), a.systemPrompt && (this.systemPrompt = a.systemPrompt), a.initialPrompts && (this.initialPrompts = a.initialPrompts), a.oncontextoverflow && (this.oncontextoverflow = a.oncontextoverflow), this.topK = a.topK || E, this.temperature = a.temperature || z, this.maxTokens = k, this.tokensLeft = k, this.tokensSoFar = 0, this.#a = [], this.systemPrompt && this.initialPrompts && Array.isArray(this.initialPrompts)) {
    ...
    this.systemPrompt && this.#a.push({
      role: u.SYSTEM,
      content: this.systemPrompt
    }), this.initialPrompts && this.#a.push(...this.initialPrompts);
  }
  updateTokensAndHistory(a) {
    this.tokensLeft = this.maxTokens - a.tokens, this.tokensSoFar = a.tokens, this.#a.push(
      {
        role: u.USER,
        content: a.prompt
      },
  ...
  async prompt(a, e = {}) {
    e.history = this.#a, e.temperature = e.temperature ?? this.temperature, e.topK = e.topK ?? this.topK, e.signal = e.signal ?? new AbortController().signal;
    const t = await w(
      a,
      e,
      (s) => this.updateTokensAndHistory(s)
    );
    let n = "";
  ...
  promptStreaming(a, e = {}) {
    return e.history = this.#a, e.temperature = e.temperature ?? this.temperature, e.topK = e.topK ?? this.topK, e.signal = e.signal ?? new AbortController().signal, w(
      a,
      e,
      (t) => this.updateTokensAndHistory(t)
    );
  }
...
class U {
  constructor() {
    this.available = g.READILY, this.defaultTopK = E, this.maxTopK = K, this.maxTemperature = D, this.defaultTemperature = z;
  }
  supportsLanguage(a) {
    return g.READILY;
  }
...
async function C(r = {}) {
  const { wasmPath: a, ...e } = r, t = (await import("./fastText.common-BVQpLcCO.js")).default;
  return await t({
    // Binding js use the callback to locate wasm for now
    locateFile: (n, s) => a ? typeof a == "string" ? a : a(n, s) : (s || "/") + n,
    ...e
  });
}
async function B(r = {}) {
...
const O = "/fastText", W = "/fastText/models", T = "train.txt", A = "test.txt", y = "model.bin", q = typeof window < "u" && !!window.document && !!window.document.createElement, H = (
  // eslint-disable-next-line @typescript-eslint/ban-ts-comment
  // @ts-expect-error
  "importScripts" in globalThis && typeof globalThis.importScripts == "function"
);
let P, S, F;
const M = (r) => new Uint8Array(r.buffer, r.byteOffset, r.length), Y = async (r) => q || H ? new Uint8Array(await (await fetch(r)).arrayBuffer()) : r.startsWith("file://") ? (P ??= (await Promise.resolve().then(() => b)).readFile, F ??= (await Promise.resolve().then(() => b)).fileURLToPath, M(await P(F(r)))) : (S ??= (await Promise.resolve().then(() => b)).request, new Promise((a, e) => {
  const t = [];
  S(
    r,
    (n) => n.on("close", () => a(M(Buffer.concat(t)))).on("data", (s) => t.push(s)).on("error", (s) => e(s))
  );
}));
class x {
  core;
  ...
  getFloat32ArrayFromHeap = (a) => {
    const e = a * Float32Array.BYTES_PER_ELEMENT, t = this.core._malloc(e), n = new Uint8Array(this.core.HEAPU8.buffer, t, e);
    return {
      ptr: n.byteOffset,
      size: a,
      buffer: n.buffer
...
`;
  ...
  saveModel() {
    ...
    return new Blob(
      [new Uint8Array(a, a.byteOffset, a.length)],
   ...
   */
  test(a, e, t) {
    const n = globalThis.fetch || fetch, s = this.ft;
    return new Promise((o, i) => {
      n(a).then((l) => l.arrayBuffer()).then((l) => {
...
const G = async (r) => {
  const { getFastTextModule: a } = r, e = await a();
  return class {
    core;
    ft;
    fs;
     ...
     */
    async loadModel(n) {
      const s = this.ft, o = await Y(n), i = new Uint8Array(o);
      return await e.FS.writeFile(y, i), s.loadModel(y), new x(s, this.core);
    }
    _train(n, s, o = {}, i = null) {
      const l = globalThis.fetch || fetch, h = this.ft;
      return new Promise((p, f) => {
        l(n).then((m) => m.arrayBuffer()).then((m) => {
          ...
          m.forEach((N) => {
            N in o && (c[N] = o[N]);
          }), c.model = e.ModelName[s], c.loss = "loss" in o ? e.LossName[o.loss] : "hs", c.thread = 1, c.input = T, h.train(c, i), p(new x(h, this.core));
        }).catch((m) => {
          f(m);
        });
      });
    ...
    trainSupervised(n, s = {}, o) {
      ...
      return new Promise((l, h) => {
        i._train(n, "supervised", s, o).then((p) => {
          l(p);
        }).catch((p) => {
          h(p);
        });
...
class d {
  ...
  async load() {
    if (!this.model) {
      ...
      }), e = new a(), t = this.modelPath;
      this.model = await e.loadModel(t);
    }
    return this.model;
  }
  async identify(a, e) {
    const t = Math.max(e || 1, 1), n = Math.min(on, t), s = (await this.load()).predict(a, n, 0);
    return typeof e > "u" ? {
...
class hn extends d {
  constructor(a = {}) {
    const e = typeof globalThis.location < "u" ? globalThis.location.origin : "";
    super({
...
(function() {
  "ai" in self && "assistant" in self.ai || typeof self < "u" && Object.defineProperty(self, "ai", {
    get() {
      return new Nn();
    },
    configurable: !0
