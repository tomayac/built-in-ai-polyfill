const y = localStorage.getItem('OPENAI_API_KEY'),
  w = (c, n, u) => {
    ...
    return new ReadableStream({
      async start(e) {
        try {
          ...
          if (!t.ok || t.status !== 200) throw new Error(await t.text());
          const f = t.body,
            p = new TextDecoder();
...
`);
            for (let o of g)
              if (
                ((o = o.replace(/data:\s*/g, '').trim()),
                !(!o || o === '[DONE]'))
              )
                try {
                  const a = JSON.parse(o);
                  if (
                    ((l = a?.usage?.total_tokens),
                    typeof a?.choices?.[0]?.delta?.content < 'u')
                  ) {
