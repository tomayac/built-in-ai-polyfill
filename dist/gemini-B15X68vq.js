const y = localStorage.getItem("GEMINI_API_KEY"), b = (l, a, f) => {
  const s = new AbortController(), c = a?.signal;
  ...
  return new ReadableStream({
    async start(t) {
      try {
        ...
        const u = e.body, p = new TextDecoder();
          ...
          for (let n of h)
            if (n = n.replace(/data:\s*/g, "").trim(), !!n)
              try {
                ...
                if (i = r?.usageMetadata?.totalTokenCount, typeof r?.candidates?.[0]?.content?.parts?.[0]?.text < "u") {
